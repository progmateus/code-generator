...
} from "../../src/createLayers.js"

import {
    createFiles
} from "../../src/createFiles";
...
describe("#Integration - Files - Files Structure", () => {
    ...
    afterAll(async () => {
        await fsPromises.rm(config.mainPath, {
            recursive: true
        })
    })
    ...
    test("Factory instance should match layers", async () => {
        ...
        const {
            default: Factory
        } = await import(factoryFile)

        const expectedInstance = new Service({
